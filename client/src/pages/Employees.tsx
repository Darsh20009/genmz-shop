...
export default function Employees() {
  ...
  const { data: salesData } = useQuery({
    queryKey: ["/api/admin/analytics/employee-sales"],
  });
    ...
    <div className="p-8 space-y-8" dir="rtl">
      {salesData && salesData.length > 0 && (
        <Card className="border-none shadow-sm">
          <CardHeader>
            <CardTitle className="text-xl font-black">مبيعات الموظفين</CardTitle>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={salesData}>
                ...
                <Legend />
                <Bar dataKey="sales" fill="#000000" name="المبيعات" />
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>

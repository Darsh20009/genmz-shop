...
export default function Profile() {
  ...
  const addAddressMutation = useMutation({
    mutationFn: async (address: any) => {
      ...
      const addresses = [...currentAddresses, { ...address, id: Math.random().toString(36).substr(2, 9) }];
  ...
  const deleteAddressMutation = useMutation({
    mutationFn: async (addressId: string) => {
      const currentAddresses = Array.isArray(user?.addresses) ? user.addresses : [];
      const addresses = currentAddresses.filter((a: any) => a.id !== addressId);
      const res = await apiRequest("PATCH", "/api/user", { addresses });
      return res.json();
    },
    onSuccess: () => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <Card className="rounded-none border-black/10 shadow-none">
          ...
          <CardContent className="pt-6 space-y-4">
            {Array.isArray(user?.addresses) && user.addresses.map((address: any) => (
